...
class Simulation(object):
  ...
  def plot_forces(self, 
    ...
    for index, simulation in enumerate(other_simulations):
      ...
      for index, force in enumerate(forces_to_plot):
        color = next(color_cycle)['color']
        line, = ax.plot(force.times, other_coefficients[index]*force.values,
                        label=' - '.join(filter(None, [simulation.description, info[index]])),
                        color=color, linestyle='--', zorder=9)
        if fill_between:
          line.remove()
          minima, maxima = force.get_extrema(order=order)
          ax.scatter(force.times[minima], other_coefficients[index]*force.values[minima],
                     c=color, marker='o', zorder=10)
          ax.scatter(force.times[maxima], other_coefficients[index]*force.values[maxima],
                     c=color, marker='o', zorder=10)
          ax.plot(force.times[minima], other_coefficients[index]*force.values[minima],
                  color='white', linestyle='-', zorder=9)
          ax.plot(force.times[maxima], other_coefficients[index]*force.values[maxima],
                  color='white', linestyle='-', zorder=9)
          times = numpy.concatenate((force.times[minima], 
                                     force.times[maxima][::-1]))
  ...
  def create_dataframe_forces(self, other_simulations=[], display_coefficients=False, silent=False):
...
class BarbaGroupSimulation(object):
  ...
  def read_fields(field_names, time_step, periodic_directions=None):
    ...
    if ['x-velocity', 'y-velocity'].any() in field_names:
  ...
  def compute_vorticity(self, periodic_directions=None):
  ...
  def subtract(self, simulation, field_name):
    """Subtract one field to another in place.

    Parameters
    ----------
    simulation: BarbaGroupSimulation object
    ...
    getattr(self, field_name).subtract(getattr(simulation, field_name))
  ...
  def plot_contour(self, field_name, 
    ...
    getattr(self, field_namereplace('-', '_')).plot_contour(directory='{}/images'.format(self.directory),
