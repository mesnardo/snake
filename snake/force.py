...
class Force(object):
  ...
  def get_deviations(self, limits=(0.0, float('inf')), order=5):
    ...
    deviations: dictionary of 2  (string, 1D array of floats) items
      Absolute deviations of the minima and maxima with respect to the mean value.
    """
  ...
  def get_strouhal(self, L=1.0, U=1.0, limits=(0.0, float('inf')), order=5):
    ...
    -------
    strouhal: dictionary
      Returns the mean Strouhal number, the number of periods used for averaging,
      the actual time-limits used, and the value over each period used.
    """
    ...
    self.strouhal = {'n-periods': minima.size-1,
                     'time-limits': (self.times[minima[0]], self.times[minima[-1]]),
                     'values': strouhals,
                     'mean': strouhals.mean()}
